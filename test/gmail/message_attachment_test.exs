...
defmodule Gmail.MessageAttachmentTest do
  ...
  setup do
    ...
    {
      :ok,
      access_token: access_token, bypass: bypass, user_id: user_id
    }
  end
  ...
  test "gets message attachment", state do
    ...
    Bypass.expect(state[:bypass], fn conn ->
      assert "/gmail/v1/users/#{state[:user_id]}/messages/#{message_id}/attachments/#{attachment_id}" ==
               conn.request_path
    ...
    assert result == %Gmail.MessageAttachment{
             size: 30,
             data: "VGhpcyBpcyBhIHRlc3QgdGV4dCBkb2N1bWVudC4K"
           }
  end
